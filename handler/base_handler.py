...
class BaseHandler(tornado.web.RequestHandler):
    ...
    def create_container_on_remote(self, node_name, docker_type, cname, container_port, advisor, run=False):
        ...
        cmd = (f"ssh {node_name} "
              ...
              "--network=host "
              f"-v /p300/docker/{cname}:/p300 "
              "-v /p300/datasets:/datasets:ro "
              f"-v /public/g_cluster/user_dir/{cname}/bin:/bin "
              f"-v /public/g_cluster/user_dir/{cname}/etc:/etc "
              f"-v /public/g_cluster/user_dir/{cname}/lib:/lib "
              f"-v /public/g_cluster/user_dir/{cname}/lib64:/lib64 "
              f"-v /public/g_cluster/user_dir/{cname}/opt:/opt "
              f"-v /public/g_cluster/user_dir/{cname}/root:/root "
    ...
    def restart_container_on_remote(self, container_name):
        ...
        username = '-'.join(container_name.split('-')[:-1])
